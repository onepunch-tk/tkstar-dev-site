...
CLASSIFY=$(printf '%s' "$CMD" | awk -v ib="$INTEGRATION_BRANCH" '
  ...
  }
  {
    ...
    if (argc < 3) {
      print class, bypass, (base_value == "" ? "-" : base_value), base_present
      exit 0
    }
...
# Restore awk sentinel (placed when base_value was empty to keep tab-field
# alignment intact across `IFS=$'\t' read`). Safe for this field only because
# git-check-ref-format(1) forbids a single-`-` refname — do not reuse this
# exact sentinel for fields where `-` is a legitimate value.
...
# Build context-dependent reason message. Anything other than BASE_PRESENT="1"
# is treated as "missing --base" so any future field-shift regression fails
# safe toward the more accurate message instead of fabricating a base name.
# The `--base=` empty-value form (BASE_PRESENT=1, BASE_VALUE="") collapses
# into the same branch — distinguishing it from a bare omission adds no value
# for the user and would render the hollow `--base ` backtick noise.
if [[ "$BASE_PRESENT" != "1" || -z "$BASE_VALUE" ]]; then
  REASON_DETAIL="Command is missing \`--base\` (or passes an empty value). \`gh pr create\` falls back to the GitHub default branch (often \`main\`), which is exactly how PR #22 leaked T006 onto \`main\` only."
else
