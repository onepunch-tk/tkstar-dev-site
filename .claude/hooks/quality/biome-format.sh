...
# Rust path: `.rs` files are formatted with `cargo fmt`, not Biome.
# Walk up to the nearest Cargo.toml; if found, run rustfmt against the edited file
# scoped to that manifest. If no Cargo.toml or no cargo binary, silently no-op.
case "$FILE_PATH" in
  *.rs)
    if CARGO_DIR=$(detect_rust_project "$FILE_PATH"); then
      if command -v cargo >/dev/null 2>&1; then
        # `cargo fmt -- <file>` formats just the named file using the workspace's rustfmt.toml.
        (cd "$CARGO_DIR" && cargo fmt -- "$FILE_PATH" 2>/dev/null) || true
      fi
    fi
    DURATION_MS=$(jq -r '.duration_ms // empty' <<<"$INPUT")
    if [[ -n "$DURATION_MS" ]]; then
      METRICS="$PROJECT_DIR/.claude/hook-metrics.jsonl"
      jq -n -c --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg hook biome-format-rs \
