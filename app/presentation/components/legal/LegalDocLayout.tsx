...
export default function LegalDocLayout({ title, version, effectiveDate, children }: Props) {
	return (
		<div className="bg-bg font-sans text-[14px] text-fg leading-[1.75]">
			<header className="border-line border-b px-[var(--spacing-gutter)] pt-[22px] pb-3">
				<h1 className="m-0 font-sans font-semibold text-[20px] text-fg tracking-[-0.01em]">
					{title}
				</h1>
				<div className="mt-2 flex flex-wrap gap-2.5 font-sans text-[11px] text-faint">
			...
			<article
				className="px-[var(--spacing-gutter)] pt-[22px] pb-20
					[&>h2]:mt-7 [&>h2]:mb-2 [&>h2]:font-sans [&>h2]:font-semibold [&>h2]:text-[14px] [&>h2]:text-fg
					[&>h3]:mt-[18px] [&>h3]:mb-1.5 [&>h3]:font-sans [&>h3]:font-semibold [&>h3]:text-[13px] [&>h3]:text-muted
					[&>p]:mb-3 [&>p]:text-fg
