		...
		<section aria-labelledby="about-career-heading" className="flex flex-col gap-3">
			...
			<ol className="m-0 flex list-none flex-col gap-0 p-0">
				{CAREER_TIMELINE.map((entry) => (
					...
					>
						<span className="font-sans text-[11px] text-faint tracking-[0.04em]">
							{entry.period}
						</span>
						<div className="flex flex-col gap-1.5">
							<div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
								<span className="font-sans font-semibold text-[13px] text-fg">{entry.company}</span>
								<span className="font-sans text-[12px] text-muted">· {entry.role}</span>
							</div>
							<ul className="m-0 flex list-none flex-col gap-1 p-0">
								{entry.points.map((point) => (
									<li
										key={point}
										className="font-sans text-[12px] text-muted leading-relaxed before:mr-2 before:text-accent before:content-['●']"
									>
										{point}
									</li>
								))}
