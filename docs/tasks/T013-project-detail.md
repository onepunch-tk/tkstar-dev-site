...
## Acceptance Criteria
- [x] `/projects/:slug` 진입 시 본문 problem/approach/results 섹션 렌더
- [x] 데스크탑 880px+에서 sticky sidebar(`min-[880px]:grid-cols-[minmax(0,1fr)_280px]`)에 meta(year/role/stack pills) + TOC가 노출
- [x] 모바일 < 880px에서는 sidebar inline (sticky 미적용)
- [x] TOC 클릭 시 해당 h2로 스크롤 (`#${slug}` anchor + `scroll-padding-top` topbar 회피)
- [x] 하단 3분할 푸터 — prev/next는 collection의 인접 항목, 가운데 [의뢰하기 →]는 `/contact`로 primary CTA
- [x] 미존재 slug 접속 시 splat 라우트로 fallback (404 응답)
- [x] 가정 A002 완료 — `toc` 필드가 velite `.transform()` 으로 자동 주입됨
