...
| **uploadMedia** | 미디어 업로드 | F022 use case — admin Editor / Media Library 가 호출하는 Workers proxy (`POST /admin/api/upload`). 클라이언트 → Workers → R2 PUT, 응답으로 public URL 반환. Cloudflare Access 게이트 안에 위치. (Phase 7.3, T034 참조) | upload file, upload image, 파일 업로드 |
